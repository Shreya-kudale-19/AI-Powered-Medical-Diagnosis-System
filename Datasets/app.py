...
background_image_url = "https://radix-communications.com/content/uploads/2023/12/Radix_BlogFeature-AI-Healthcare.jpg"  # Replace with your image URL
...
# Global CSS for Result Formatting
st.markdown(
    """
    <style>
...
# Custom CSS to change the selectbox label color
st.markdown("""
    <style>
...
def display_input(label, tooltip, key, type="text"):
    st.markdown(f"<label style='font-size: 16px; font-weight: bold; color: #ffffff; margin-bottom: -50px; display: block;'>{label}</label>", unsafe_allow_html=True)
    if type == "text":
        return st.text_input("", key=key, help=tooltip)
    elif type == "number":
        return st.number_input("", key=key, help=tooltip, step=1)

# step=1 → Ensures that the user can increment or decrement values by 1.
# i.e: Clicking the up/down buttons increases/decreases the value by 1.
...
if selected == 'Diabetes Prediction':
    ...
    st.markdown("<h1 style='text-align: center; font-size: 35px; font-weight: bold; color: #ffffff; font-family: Georgia, serif;'>Diabetes Prediction</h1>", unsafe_allow_html=True)

    st.markdown("<p style='text-align: center; font-size: 15px; font-weight: bold; color: #ffffff; margin-bottom: 50px; font-family: Verdana, sans-serif;'>Enter the following details to predict diabetes:</p>", unsafe_allow_html=True)
    ...
    with col1:
        ...
        DiabetesPedigree = display_input('Diabetes Pedigree Function value', 'Enter diabetes pedigree function value', 'DiabetesPedigreeFunction', 'number')
    ...
    if st.button('Diabetes Test Result'):
        diabetes_prediction = models['diabetes'].predict([[Pregnancies, Glucose, BloodPressure, SkinThickness, Insulin, BMI, DiabetesPedigree, Age]])
        if diabetes_prediction[0] == 1:
            # Red box for positive case
            st.error("The person is Diabetic")  # Red box for positive case
        else:
            st.success("The person is not diabetic") 
             # Green box for negative case
...
if selected == 'Heart Disease Prediction':
    st.markdown("<h1 style='text-align: center; font-size: 35px; font-weight: bold; color: #ffffff; font-family: Georgia, serif;'>Heart Disease Prediction</h1>", unsafe_allow_html=True)
    ...
    if st.button('Heart Disease Test Result'):
        ...
        if heart_prediction[0] == 1:
            st.error("The person has heart disease")  # Red box for positive case
        else:
            st.success("The person does not have heart disease")  # Green box for negative case
...
if selected == "Parkinsons Prediction":
    st.markdown("<h1 style='text-align: center; font-size: 35px; font-weight: bold; color: #ffffff; font-family: Georgia, serif;'>Parkinsons Prediction</h1>", unsafe_allow_html=True)
    ...
    if st.button("Parkinson's Test Result"):
        ...
        if parkinsons_prediction[0] == 1:
            st.error("The person has Parkinson’s disease")  # Red box for positive case
        else:
            st.success("The person does not have Parkinson’s disease")  # Green box for negative case
...
if selected == "Lung Cancer Prediction":
    st.markdown("<h1 style='text-align: center; font-size: 35px; font-weight: bold; color: #ffffff; font-family: Georgia, serif;'>Lung Cancer</h1>", unsafe_allow_html=True)
    ...
    if st.button("Lung Cancer Test Result"):
        ...
        if lungs_prediction[0] == 1:
            st.error("The person has lung cancer")  # Red box for positive case
        else:
            st.success("The person does not have lung cancer")  # Green box for negative case
...
if selected == "Hypo-Thyroid Prediction":
    st.markdown("<h1 style='text-align: center; font-size: 35px; font-weight: bold; color: #ffffff; font-family: Georgia, serif;'>Hypo-Thyroid</h1>", unsafe_allow_html=True)
    ...
    # Prediction Button
    hypo_thyroid_diagnosis = ''
    if st.button("Hypo-Thyroid Test Result"):
        hypo_thyroid_prediction = models['thyroid'].predict([[age, sex, on_thyroxine, tsh, t3_measured, t3, tt4]])
        if hypo_thyroid_prediction[0] == 1:
            st.error("The person has hypo-thyroid")  # Red box for positive case
        else:
            st.success("The person does not have hypo-thyroid")  # Green box for negative case
